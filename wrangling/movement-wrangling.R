...
tracking <- fread('G:/My Drive/MSBA/ML/Final Project/nfl-play-predictor/data/tracking_week_1.csv')

# Will use later to add player names to results to make it understandable
players_full <- fread('G:/My Drive/MSBA/ML/Final Project/nfl-play-predictor/data/players.csv')
...
names(res_db) <- c(names(unlist(start_pos_off)), names(unlist(snap_pos_off)), 
                   names(unlist(start_pos_def)), names(unlist(snap_pos_def)))

# Movement wrangling
# SUPER UGLY WRANGLING WARNING
library(dplyr)
library(tidyr)
library(reshape2)
the_play <- res_db[11, ]
...
library(ggrepel)
library(gganimate)
play_data_anim <- tracking[which(tracking$playId == play_id &
                            tracking$gameId == game_id),]
p <- ggplot(play_data_anim, aes(y = y, x = x, color = club)) +
        geom_point(aes(alpha = frameId)) +
        dark_theme_bw() +
        geom_text_repel(data = play_data_anim[!is.na(play_data_anim$event) &
                                            play_data_anim$club == "football",], aes( label = event))
panimate <- p + transition_time(frameId)

# Variable Selection
# Response variable - expectedPointsAdded from plays data
# 0a. Tracking data that I want to use - a(at snap), s(at snap), dir(at snap)
# 0b. Play data that I want to use - quarter, down, offenseFomation, recieverAlignment
# 1. Euclidian distance between line set and ball snap
the_play_df$dist_moved <- sqrt((the_play_df$x_snap - the_play_df$x_start)^2 + 
                                 (the_play_df$y_snap - the_play_df$y_start)^2)

# 2. Angle of movement
library(units)
the_play_df$angle_moved <- atan((the_play_df$y_snap - the_play_df$y_start) / 
                                 (the_play_df$x_snap - the_play_df$x_start)) * (180/pi)
